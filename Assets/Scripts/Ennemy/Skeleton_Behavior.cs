...
public class Skeleton_Behavior : MonoBehaviour
{
    ...
    /// Décrit la durée de l'invulnaribilité
    /// </summary>
    public const float _delaisSpeedUp = 4f;
    /// <summary>
    ...
    /// Représente le moment où l'invulnaribilité a commencé
    /// </summary>
    private float _tempsDebutSpeedUp;
    /// <summary>
    ...
    private void Update()
    {
        ...
        if (Time.fixedTime > _tempsDebutSpeedUp + _delaisSpeedUp)
            FindObjectOfType<EnnemyPatrol>().setSpeed(3f);
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.tag.Equals("Player"))
        {
            if (!_invulnerable)
            {
                ...
                _invulnerable = true;
                _tempsDebutSpeedUp = Time.fixedTime;
                FindObjectOfType<EnnemyPatrol>().setSpeed(5f);
            }
        }
    }
}